...
?>
    ...
    $rs = Database::search("SELECT * FROM `invoice` JOIN `cutomer_details` ON `invoice`.`cutomer_details_id` = `cutomer_details`.`id` JOIN `pay_method` ON `invoice`.`pay_method_mid` = `pay_method`.`mid` WHERE `invoice`.`ad_order_s_ad_o_s` = '" . $cat . "' AND `invoice`.`order_status_id`='2' ORDER BY `invoice`.`date` DESC");
